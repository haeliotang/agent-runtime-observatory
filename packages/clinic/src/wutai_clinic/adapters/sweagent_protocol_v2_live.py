...
SWEAGENT_PROTOCOL_V2_LIVE_SINGLE_PHASE = "6.protocol_v2_sweagent_live_single"
SWEAGENT_PROTOCOL_V2_LIVE_SINGLE_VERSION = "phase6_protocol_v2_sweagent_live_single_v1"
BOUNDARY = (
    "This package plans or executes one Protocol v2 SWE-agent live-single adapter arm. "
...
ProtocolV2ArmType = Literal["control", "treatment"]


@dataclass(frozen=True)
class SWEAgentProtocolV2LiveSingleSpec:
    config_path: Path
    output_dir: Path
    protocol: ProtocolV2
    replay_actions: list[dict[str, Any] | str] = field(default_factory=list)
    arm_type: ProtocolV2ArmType = "treatment"
    execute: bool = False
    source_task_id: str | None = None
    pair_id: str | None = None
    require_official_eval: bool = False
    # Explicit opt-out of the replay-prefix cost guard (task10 replay-free probe).
    allow_empty_replay: bool = False
    # Prescription v3 (task14): constraint hook detects but never enforces.
    observe_only: bool = False


@dataclass
class SWEAgentProtocolV2Attachment:
    hook: ProtocolV2ConstraintHook | None
    original_model: Any
...
class SWEAgentProtocolV2RunSingleAdapter:
    ...
    def attach(
        ...
        *,
        ...
        if arm_type == "treatment":
            ...
            agent.add_hook(hook)
        return SWEAgentProtocolV2Attachment(
            hook=hook,
...
def run_sweagent_protocol_v2_live_single(
    *,
    ...
    replay_ready = len(spec.replay_actions) > 0 or spec.allow_empty_replay
    gates = {
    ...
    if should_run:
        ...
        try:
            ...
            adapter = SWEAgentProtocolV2RunSingleAdapter(policy=policy)
        ...
        except ProtocolV2ConstraintViolation as exc:
    ...
    if not spec.execute:
        decision = "protocol_v2_live_single_planned_no_run"
    elif not authorized:
        decision = "protocol_v2_live_single_blocked_needs_ack"
    elif not config_exists:
        decision = "protocol_v2_live_single_blocked_missing_config"
    elif not replay_ready:
        decision = "protocol_v2_live_single_blocked_missing_replay_actions"
    elif run_error is not None:
        decision = "protocol_v2_live_single_run_failed"
    elif constraint_block_event is not None:
        decision = "protocol_v2_live_single_constraint_blocked"
    else:
    ...
    if not spec.execute:
        ...
        recommended_next_step = "assemble_protocol_v2_pair_if_both_arms_complete"

    protocol_path = spec.output_dir / "protocol_v2_live_single_protocol.json"
    replay_path = spec.output_dir / "protocol_v2_live_single_replay_actions.json"
    events_path = spec.output_dir / "protocol_v2_live_single_events.jsonl"
    report_path = spec.output_dir / "protocol_v2_live_single_report.json"
    manifest_path = spec.output_dir / "protocol_v2_live_single_manifest.json"
    ...
    events = [{"event_type": "model_query", **event} for event in model_events] + [
        {"event_type": "protocol_v2_constraint", **event} for event in hook_events
    ]
    write_jsonl(events_path, events)
    report = generate_report(
        phase=SWEAGENT_PROTOCOL_V2_LIVE_SINGLE_PHASE,
        decision=decision,
        gate_results=gates,
        extras={
            "version": SWEAGENT_PROTOCOL_V2_LIVE_SINGLE_VERSION,
            ...
            "empty_replay_authorized": spec.allow_empty_replay,
            "observe_only": spec.observe_only,
            "would_block_count": sum(1 for event in hook_events if event.get("would_have_blocked")),
    ...
    manifest["version"] = SWEAGENT_PROTOCOL_V2_LIVE_SINGLE_VERSION
