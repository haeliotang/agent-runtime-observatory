...
SWEAGENT_PROTOCOL_V1_LIVE_PAIR_PHASE = "6.protocol_v1_sweagent_live_pair"
SWEAGENT_PROTOCOL_V1_LIVE_PAIR_VERSION = "phase6_protocol_v1_sweagent_live_pair_v1"
BOUNDARY = (
    "This package combines two completed Protocol v1 SWE-agent live-single arms into a "
...
class SWEAgentProtocolV1LivePairSpec:
    control_dir: Path
    treatment_dir: Path
    output_dir: Path
    control_patch: Path
...
def _constraint_events(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [event for event in events if event.get("event_type") == "protocol_v1_constraint"]
...
def _outcomes_complete_or_absent(spec: SWEAgentProtocolV1LivePairSpec) -> bool:
    return (
...
def run_sweagent_protocol_v1_live_pair(
    *,
    ...
    control_report_path = spec.control_dir / "protocol_v1_live_single_report.json"
    treatment_report_path = spec.treatment_dir / "protocol_v1_live_single_report.json"
    control_events_path = spec.control_dir / "protocol_v1_live_single_events.jsonl"
    treatment_events_path = spec.treatment_dir / "protocol_v1_live_single_events.jsonl"
    control_protocol_path = spec.control_dir / "protocol_v1_live_single_protocol.json"
    treatment_protocol_path = spec.treatment_dir / "protocol_v1_live_single_protocol.json"
    control_replay_path = spec.control_dir / "protocol_v1_live_single_replay_actions.json"
    treatment_replay_path = spec.treatment_dir / "protocol_v1_live_single_replay_actions.json"
    ...
    control_patch_archive = spec.output_dir / "control" / "sweagent_protocol_v1_live_single.patch"
    treatment_patch_archive = (
        spec.output_dir / "treatment" / "sweagent_protocol_v1_live_single.patch"
    )
    ...
    gates = {
        ...
        == "protocol_v1_live_single_run_completed"
        ...
        "treatment_constraint_not_blocked": treatment_report.get("constraint_blocked") is False
        and not any(event.get("blocked") is True for event in treatment_constraints),
    ...
    if not structural_passed:
        decision = "protocol_v1_live_pair_blocked"
    elif outcomes_present:
        decision = "protocol_v1_live_pair_outcome_label_ready"
    else:
        decision = "protocol_v1_live_pair_ready_pending_official_eval"
    ...
    report_path = spec.output_dir / "protocol_v1_live_pair_report.json"
    summary_path = spec.output_dir / "protocol_v1_live_pair_summary.jsonl"
    manifest_path = spec.output_dir / "protocol_v1_live_pair_manifest.json"
    report = generate_report(
        phase=SWEAGENT_PROTOCOL_V1_LIVE_PAIR_PHASE,
        decision=decision,
        gate_results=gates,
        extras={
            "version": SWEAGENT_PROTOCOL_V1_LIVE_PAIR_VERSION,
    ...
    manifest["version"] = SWEAGENT_PROTOCOL_V1_LIVE_PAIR_VERSION
...
__all__ = [
    "SWEAGENT_PROTOCOL_V1_LIVE_PAIR_PHASE",
    "SWEAGENT_PROTOCOL_V1_LIVE_PAIR_VERSION",
