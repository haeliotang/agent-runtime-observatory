...
class TestCollectPairOutcomes:
    def test_valid_scorecard(self, tmp_path: Path) -> None:
        _make_scorecard(tmp_path, "myrepo__myrepo-1234", "both_unresolved_trigger_hit_pair_no_uplift")
    ...
    def test_stratum_detection_v2(self, tmp_path: Path) -> None:
        _make_scorecard(tmp_path, "sphinx-doc__sphinx-8474", "both_unresolved_trigger_hit_pair_no_uplift")
        outcomes = collect_pair_outcomes(tmp_path)
        found = [o for o in outcomes if o.get("instance_id") != "_unparsed"]
...
class TestBuildHtml:
    def _make_dag(self) -> dict[str, Any]:
        return {"nodes": [], "edges": [], "truncated": False, "truncated_count": 0, "scan_root": "/tmp"}
    ...
    def test_truncation_note_in_html(self, tmp_path: Path) -> None:
        ...
        for i in range(160):
            ...
            (sub / f"official_eval_report_{i}.json").write_text(
                json.dumps({"effect_label": "both_unresolved_trigger_hit_pair_no_uplift",
                             "pair_id": f"pair_{i}", "source_task_id": f"repo-{i}"}),
                encoding="utf-8",
            )
...
class TestWriteHtmlReport:
    def test_writes_file(self, tmp_path: Path) -> None:
        ...
        evidence.mkdir()
        _make_scorecard(evidence, "myrepo__myrepo-9999", "both_unresolved_trigger_hit_pair_no_uplift")
    ...
    def test_effect_label_present_when_data_exists(self, tmp_path: Path) -> None:
        evidence = tmp_path / "evidence"
        evidence.mkdir()
        _make_scorecard(evidence, "sphinx-doc__sphinx-8474", "both_unresolved_trigger_hit_pair_no_uplift")
    ...
    def test_result_dict_keys(self, tmp_path: Path) -> None:
        ...
        output = tmp_path / "report.html"
        result = write_html_report(evidence, output)
        for key in ("output_path", "pairs_found", "nodes_found", "edges_found",
                    "truncated", "generated_at"):
