...
| 10 | Debt is **consumed** by Attestations, per item, guarded, race-free, and content-bound: `clears_decisions` binds to specific `needs_review` decisions; clearing is bound to a **versioned canonical subject** (schema evolution doesn't stale; different content reopens); a `seat_id` is **required** to clear and must be declared; `Goal.owner_seat_id` must be a declared seat; blank identity / reject-clearing / duplicate ids refused; open/cleared/stale are **store-derived gauges** (no counter to double-count) | `compute_review_debt()` + `run_subject_digest()` (versioned) in `aro_schema/review_debt.py`; `Script` owner-seat validator; `refresh_review_debt_gauges()`; `GET /api/runs/{id}/review-debt?status=open`; `aro_review_debt_open`/`_stale`/`_oldest_open_age_seconds`; tests incl. blank/forged/duplicate/**24-concurrent**/overwrite-reopens/seat-required/schema-stable-digest (CI: `unit`, `integration`) | ✅ |
