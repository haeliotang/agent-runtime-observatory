...
| **G2** | **Demo-grade security defaults** — the API is unauthenticated, CORS is open to `localhost:5173`, and rate limiting is the only guard | The substrate is **not internet-facing**; it is a reference/local deployment. Do not expose `aro_api` publicly without an auth layer in front | `SECURITY.md` (pending) states this boundary |
| **G3** | The exfiltration example ships a **fixture `.env`** (`examples/policy-violation-run/workspace/.env`) | It contains obviously fake values, but an unlabeled secret-shaped file invites misreading | to be labeled `FAKE` explicitly + redaction note (pending, same change as `SECURITY.md`) |
