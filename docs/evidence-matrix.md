...
| 10 | Debt is **consumed** by Attestations, per item, guarded, and race-free: `clears_decisions` binds to specific `needs_review` decisions; clearing is digest-bound (overwritten run reopens + flags stale); blank identity, unknown seats, reject-clearing, duplicate ids refused; open/cleared/stale are **store-derived gauges** (no counter to double-count under concurrency) | `compute_review_debt()` + `run_subject_digest()` in `aro_schema/review_debt.py`; `refresh_review_debt_gauges()` in `aro_telemetry/metrics.py`; `GET /api/runs/{id}/review-debt?status=open`; `aro_review_debt_open`/`_stale`/`_oldest_open_age_seconds`; tests incl. blank/forged/duplicate/**24-concurrent** and overwrite-reopens (CI: `unit`, `integration`) | ✅ |
